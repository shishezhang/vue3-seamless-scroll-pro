import { computed, CSSProperties, defineComponent, onBeforeMount, onMounted, ref, watch, nextTick, getCurrentInstance } from "vue";
  ...
  setup(_props, { slots, emit, attrs }) {
    ...
    const realBoxStyle = computed(() => {
      return {
        ...
        // @ts-ignore
        transition: `all ${typeof props.ease === "string" ? props.ease : "cubic-bezier(" + props.ease.x1 + "," + props.ease.y1 + "," + props.ease.x2 + "," + props.ease.y2 + ")"} ${
          props.delay
        }ms`,
        overflow: "hidden",
    ...
    const animation = (_direction: "up" | "down" | "left" | "right", _step: number, isWheel?: boolean) => {
      reqFrame.value = requestAnimationFrame(function () {
        ...
        if (_direction === "up") {
          ...
          debugger
            _scrollIndex.value++;
            if(_scrollIndex.value > props.list.length){
              _scrollIndex.value = 1
            }
            emit("scroll", _scrollIndex.value);
          }
